...
IRx        EQU    GP3    ;INPUT ONLY PIN(IR Rx 38kHz)
 MESSG /*PIC10F200 is selected as TARGET*/
 endif
;==========================================================================
 ifdef __10F202
 include "p10F202.inc"
...
SDA        EQU    GP2    ;SDA pin of the I2C Bus
IRx        EQU    GP3    ;INPUT ONLY PIN(IR Rx 38kHz)
 MESSG /*PIC10F202 is selected as TARGET*/
 endif
;==========================================================================
 ifdef __10F204
 include "p10F204.inc"
...
SDA        EQU    GP2    ;SDA pin of the I2C Bus
IRx        EQU    GP3    ;INPUT ONLY PIN(IR Rx 38kHz)
 MESSG /*PIC10F204 is selected as TARGET*/
 endif
;==========================================================================
 ifdef __10F206
 include "p10F206.inc"
...
SDA        EQU    GP2    ;SDA pin of the I2C Bus
IRx        EQU    GP3    ;INPUT ONLY PIN(IR Rx 38kHz)
 MESSG /*PIC10F206 is selected as TARGET*/
 endif
;==========================================================================
 ifdef __10F220
 include "p10F220.inc"
...
SDA        EQU    GP2    ;SDA pin of the I2C Bus
IRx        EQU    GP3    ;INPUT ONLY PIN(IR Rx 38kHz)
 MESSG /*PIC10F220 is selected as TARGET*/
 endif
;==========================================================================
 ifdef __10F222
 include "p10F222.inc"
...
SDA        EQU    GP2    ;SDA pin of the I2C Bus
IRx        EQU    GP3    ;INPUT ONLY PIN(IR Rx 38kHz)
 MESSG /*PIC10F222 is selected as TARGET*/
 endif
;========================================================================== 
 ifdef __12F508
 include "p12F508.inc"
...
SDA        EQU    GP5    ;SDA pin of the I²C Bus
 ...
 errorlevel +202,+205,+207,+301,+302,+305,+306,+224
;==========================================================================
		CBLOCK RAM
;==========================================================================
...
freqL EQU ((freqB&3) << 6) ; Shift channel selection for matching register 0x03
 ...
 ifdef __10F206
    BCF	   CMCON0,CMPON  ;Disable Digital Comparator function
 endif
 ifdef __10F220
    BCF    ADCON0,ADON   
 endif
 ifdef __10F222
    BCF    ADCON0,ADON   ;Disable Analog to Digital function
 endif
 ifdef __12F510
    BCF    CM1CON0,C1ON     ;Disable Digital Comparator function
    BCF    ADCON0,ADON      ;Disable Analog to Digital function
 endif
...
INIT:
    ...
    MOVLW  0x3F           ;changed from 0x0F for 12F5xx into 'port' register
...
;DEFAULT STARTING:        ;Default starting point after init

    movlw  volume_init    ;0x83
    movlw  volume
    movlw  freqL          ;91.90 MHz
    movwf  frequency_l
    movlw  freqH          ;91.90 MHz
    movwf  frequency_h
;
